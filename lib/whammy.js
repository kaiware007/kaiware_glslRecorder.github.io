...
window.Whammy = (function(){
	...
	function checkFrames(frames){
		...
		for(var i = 1; i < frames.length; i++){
			if(frames[i].width != width) throw "Frame " + (i + 1) + " has a different width : " + width + " : " + frames[i].width;
			if(frames[i].height != height) throw "Frame " + (i + 1) + " has a different height : " + height + " : " + frames[i].height;;
